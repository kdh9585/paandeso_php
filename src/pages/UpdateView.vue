      ...
      <div class="card-body">
        <form @submit.prevent="onSubmit">
            <div class="form-group">
              ...
              <input type="text"
                  class="form-control form-control-sm" name="title" id="title" aria-describedby="helpId" placeholder="" v-model="todo.title">
...
export default {
  setup(){
    ...
    const getInfo = () => {
      fetch(`http://hwik95.dothome.co.kr/data_read_id.php?id=${route.params.id}`)
        .then(res => res.json())
        .then(data => {
            // console.log(data);
    ...
    const onSubmit = ()=> {
      fetch(`http://hwik95.dothome.co.kr/data_update.php?id=${todo.id}&title=${todo.title}&body=${todo.body}&complete=${todo.complete}`)
      .then(res => res.json())
      .then(data => {
