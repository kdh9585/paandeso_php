        ...
        <form @submit.prevent="onSubmit">
          <div class="form-group">
            ...
            <input type="text" class="form-control form-control-sm" name="title" id="title" aria-describedby="helpId"
              placeholder="" v-model="todo.title">
...
<script>
  ...
  export default {
    setup() {
      ...
      // todo 와 originalTodo 를 비교한다.
      // 그 결과가 true 인지 false 인지 비교한다.
      ...
      const onSubmit = () => {
        ...
        fetch(
            `http://hwik95.dothome.co.kr/data_update.php?id=${todo.id}&title=${todo.title}&body=${todo.body}&complete=${todo.complete}`
            )
          .then(res => res.json())
          .then(data => {
