        ...
        <div class="card">
            <div class="card-header">
                ...
                <div class="input-group mr-2 search">
                    <input class="form-control" placeholder="검색" v-model="searchTxt" @keyup.enter="getTotalSearch()">
                </div>
                ...
                <table class="table">
                    ...
                    <tbody>
                        <tr v-for="(item, index) in todos" :key="index">
                            ...
                            <td>
                                <input type="checkbox" class="ml-2 mr-2" :id="item.id" v-model="item.active"
        ...
        <nav aria-label="Page navigation example" v-show="page_total > 1">
            <ul class="pagination">
                ...
                <li class="page-item" v-for="item in page_total" :key="item">
                    <a class="page-link" href="#" @click="getInfo(item)"
                        :class="page_now == item ? 'active' : '' ">{{item}}</a>
                </li>
    ...
    export default {
        ...
        setup() {
            ...
            const getInfo = (_page = 1) => {
                ...
                fetch(
                        `http://hwik95.dothome.co.kr/data_read.php?page_now=${page_now.value}&data_count=${data_count}`)
                    .then(res => res.json())
                    .then(data => {
                        // 배열데이터를 보관한다.    
            ...
            const deleteTodo = () => {
                // console.log(_id);
                fetch(`http://hwik95.dothome.co.kr/data_delete.php?id=${deleteId.value}`)
                    .then(res => res.json())
                    .then(data => {
                        // console.log(data);
                        // 목록갱신
            ...
            const toggleTodo = (_obj) => {
                ...
                fetch(
                        `http://hwik95.dothome.co.kr/data_update.php?id=${_obj.id}&title=${_obj.title}&body=${_obj.body}&complete=${_obj.complete}`)
                    .then(res => res.json())
                    .then(data => {
                        console.log(data)
                    })
            ...
            // 전체 데이터 수 받아오기
            const getTotal = () => {
                fetch(`http://hwik95.dothome.co.kr/data_total.php`)
                    .then(res => res.json())
                    .then(data => {
                        // 전체 데이터 수 갱신
            ...
            getTotal();
            // 현재 생성된 목록이 검색으로 된것 인지
            // 아니면 검색없이 일반적인 목록인지를 구분하는 변수를 하나 만들자.
            const searchActive = ref(false);
            // 검색 기능 구현
            const searchTxt = ref('');
            // 잦은 검색 php 요청을 하는 것을 방지 용도
            ...
            const getTotalSearch = () => {
                ...
                clearTimeout(searchTimer);
                fetch(`http://hwik95.dothome.co.kr/data_total_search.php?title=${searchTxt.value}`)
                    .then(res => res.json())
                    .then(data => {
                        // 검색어에 해당하는 데이터 개수
                        data_total.value = data.total;
            ...
            // 검색에 해당되는 내용을 목록으로 가지고 오는 php 를 실행
            // data_read_search.php
            const searchTodo = () => {
                fetch(
                        `http://hwik95.dothome.co.kr/data_read_search.php?page_now=${page_now.value}&data_count=${data_count}&title=${searchTxt.value}`)
                    .then(res => res.json())
                    .then(data => {
                        // 검색된 배열을 받아서 todos 를 업데이트 한다.
                        todos.value = data.result;
                        // complete 를 '0' 
                        for (let item of todos.value) {
                            if (item.complete === '0') {
